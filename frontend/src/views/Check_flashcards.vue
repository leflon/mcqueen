  ...
  <div class="Check-flascard-page">
    ...
    <div class="Search-bar">
      <div class="Search-input">
        <input
          type="text"
          placeholder="Search a flashcard"
    ...
    <button class="Create-button" @click="() => router.push('/creation_mode')">
      Create flashcard
    </button>
    ...
    <div class="Container-Category-Cards">
      <div
        class="Category-card"
        v-for="value in filtered_list_flascards"
        :key="value['id']"
      >
        <h4>{{ value['name'] }}</h4>
        ...
        <div class="Practice-Edit-buttons">
          <button @click="() => router.push(`/practice/${value['id']}`)">
            Practice
          </button>
          <button @click="() => router.push(`/edit_mode/${value['id']}`)">
            Edit
          </button>
        </div>
      </div>
      <h2
        v-if="filtered_list_flascards.length === 0 && list_flascards.length > 0"
      >
        No results
      </h2>
...
async function get_lists() {
  try {
    ...
    const listsWithQuestions = await Promise.all(
