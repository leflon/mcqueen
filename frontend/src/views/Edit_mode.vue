<template>
    <div class="Edit-mode-page">
        <div v-if="others_flashcards_lists.length!==0" class="Side-bar" :class="{ display: hide_sidebar, hidden: !hide_sidebar}">
            <div class="Side-bar-content" :class="{ display: hide_sidebar, hidden: !hide_sidebar }">
                <h2>Your FlashCards</h2>

                <ul>
                    <li v-for="value in others_flashcards_lists" :key="value['id']" @click="navigate_to_other_lists(value['id'])" style="cursor: pointer;">
                        <h3>{{ value['name'] }}</h3>
                    </li>
                </ul>
        ...
        <div class="Edit-Mode-section" :class="{ display: hide_sidebar, hidden: !hide_sidebar}">
            ...
            <div class="Edit-Mode-Controls">
                <input type="text" placeholder="Change flashcard title..." v-model="new_list_title">
            ...
            <div class="Card-section">
                ...
                <div class="Single-Card-section" v-for="value in flashcards" :key="value['id']">
                    <div >
                        <p>Question</p>
                    ...
                    <div>
                        <div class="Card-buttons">
                            <button @click="edit_single_flashcard(value['id'],value['question_text'],value['answer_text'])">Save</button>
                            <button @click="delete_single_flashcard(value['id'])">Delete</button>
                        </div>
                    </div>
                </div>
            </div>
...
async function get_others_list_info() {
    try {
        ...
        others_flashcards_lists.value = data.lists.filter((card: any) => card["id"] !== id);
...
async function edit_single_flashcard(flascard_id: string, new_question: string, new_answer: string) {
...
<style scoped>
    ...
    .Add-card-section, .Single-Card-section{
        width: 100%;
        min-height: 120px;
        display: flex;
        align-items: center;
    ...
    .Add-card-section > div, .Single-Card-section > div{
        width: 30%;
        display: flex;
        flex-direction: column;
        justify-content: center;
